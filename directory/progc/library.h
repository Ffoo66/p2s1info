...
// -t process tree to count how many times the city (char* city) is visited (int count) and how many times it's a departure city (int fcount), pTreei a checks if the ID wasn't already linked to the city
	struct Treet1* l;
	struct Treet1* r;
	char* city;
	int count;
...
// inserts a route ID and distance into an AVL, if the ID is already in the tree, the distance will add to the ID's distance value
...
// inserts a city and a route ID into an AVL, the right counters would be modified depending on whether a city/ID duo already exists in the AVL or not
...
// inserts a city and a route ID into an AVL sorted by alphabetical order
...
// creates a tree which stores the route ID and a few statistics (min distance per step, max distance per step, average distance per step)
...
// fills a queue with the tree which was sorted by route ID and calculates the average distance and the max-min value
